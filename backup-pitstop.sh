#!/usr/bin/env bash
# ================================================================
# backup-pitstop.sh — Backup do banco pitstopmanager_db
...
DB_NAME="pitstopmanager_db"
DB_USER="pitstopmanager_user"
BACKUP_DIR="${BACKUP_DIR:-/opt/pitstop/backups}"
RETENTION_DAYS=30

# ── Timestamp ─────────────────────────────────────────────────
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_FILE="${BACKUP_DIR}/pitstopmanager_${TIMESTAMP}.sql.gz"
...
find "${BACKUP_DIR}" -name "pitstopmanager_*.sql.gz" -mtime "+${RETENTION_DAYS}" -delete
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Limpeza concluída."

# ── Listar backups disponíveis ────────────────────────────────
echo ""
echo "Backups disponíveis em ${BACKUP_DIR}:"
ls -lh "${BACKUP_DIR}"/pitstopmanager_*.sql.gz 2>/dev/null || echo "(nenhum)"
